...
c     ################################################################
c     ##                                                            ##
c     ##  program testlambdagrad  --  test lambda derivatives       ##
c     ##                                                            ##
c     ################################################################
c
c
c     "testlambdagrad" computes the analytical and numerical
c     gradient vectors of the potential energy function with respect
c     to Cartesian coordinates and lambda for a given lambda
c
c
      program testlambdagrad
      implicit none
      include 'sizes.i'
      include 'atoms.i'
      include 'bath.i'
...
c
c     decide whether to do lambda sweep
c
      dosweep = .false.
      call nextarg (answer,exist)
      if (.not. exist) then
         write (iout,5)
   5    format (/,' Do lambda sweep [N] :  ',$)
         read (input,6)  record
   6    format (a120)
         next = 1
      ...
      call upcase (answer)
      if (answer .eq. 'Y')  dosweep = .true.     
      
      if (dosweep) then
c        lambda = 0.0d0
        print *, ""
        print *, "Running DTenergy or gradient at lambda"
        print *, "in intervals of 0.01"
        print *, "lambda, energy, dudl"
        do while (lambda .le. 1.0d0)
        if (osrwon) then 
          DTu0 = DTenergy(lambda)
        else 
          call gradient (DTu0, detot)
        end if
        print *, lambda, DTu0, dudl
...
c
c     decide whether to do testing of endpoints
c
      doendpoints = .false.
      call nextarg (answer,exist)
      if (.not. exist) then
         write (iout,7)
   7    format (/,' Do endpoints [N] :  ',$)
         read (input,8)  record
   8    format (a120)
         next = 1
         call gettext (record,answer,next)
      ...
      call upcase (answer)
      if (answer .eq. 'Y')  doendpoints = .true. 
...
c
   ...
   10    format (a120)
      ...
      if (dotime) then
        print *, ""
        print *, "Running 100 iterations of DTenergy"
        lambda = 0.8d0
        osrwon = .true.
        t0=omp_get_wtime()
        do i = 1,100
...
c        print *, 'd2udl2',d2udl2
      ...
      if (donumer) then
        print *, 'Numeric'
        print *, 'dudl',ndudl,'d2udl2',nd2udl2
         ...
         if (dofull .and. use(i)) then
            ...
            do j = 1, 3
               if (doanalyt) then
                  ...
                  print *, 'desum', desum(j,i),'d2udl(r)',d2udl(j,i)
...
c
               if (donumer) then
                  ...
                  print *, 'detot', ndetot(j,i),'d2udl(r)',nd2udl(j,i)
...
c
  ...
  360          format (' Anlyt',2x,i8,7x,3f12.4,2x,f12.4)
         ...
         if (donumer .and. use(i)) then
            ndenorm(i) = nd2udl(1,i)**2 + nd2udl(2,i)**2 +
     &                        nd2udl(3,i)**2
            ntotnorm = ntotnorm + ndenorm(i)
            ndenorm(i) = sqrt(ndenorm(i))
            if (digits .ge. 8) then
               write (iout,370)  i,(nd2udl(j,i),j=1,3),ndenorm(i)
  370          format (' Numer',i8,1x,3f16.8,f16.8)
            else if (digits .ge. 6) then
               write (iout,380)  i,(nd2udl(j,i),j=1,3),ndenorm(i)
  380          format (' Numer',2x,i8,3x,3f14.6,2x,f14.6)
            else
               write (iout,390)  i,(nd2udl(j,i),j=1,3),ndenorm(i)
